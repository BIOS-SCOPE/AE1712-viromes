...
df = read_tsv('data/GOV2_merged_dataframe.tsv.gz') %>% filter(sample_type %in% c('GOV2', 'BATS'))
...
ggsave('plots/GOV2-combined-40pct.pdf', plot = p, device=pdf, width=210*2, height=297, units='mm', limitsize = FALSE)
p2 = make_viral_plot(df, cutoff=70)
ggsave('plots/GOV2-combined-70pct.pdf', plot = p2, device=pdf, width=210*2, height=297, units='mm', limitsize = FALSE)
